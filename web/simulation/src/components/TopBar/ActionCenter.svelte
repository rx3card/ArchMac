<script lang="ts">
	...
	import BrightnessIcon from '~icons/mdi/brightness-7';
	import VolumeIcon from '~icons/mdi/volume-high';

	import { colors } from '$lib/configs/theme/colors.config';
	import { wallpapers_config } from '$lib/configs/wallpapers/wallpaper.config';
	import { apps } from '$lib/state/apps.svelte.ts';
	import { should_show_notch } from '$lib/state/menubar.svelte';
	import { preferences } from '$lib/state/preferences.svelte.ts';
	import { display } from '$lib/state/display.svelte.ts';
...
>
	...
	>
		<ActionCenterTile grid={[1, 1]} role="region">
			<ControlSlider bind:value={display.brightness} label="Brillo">
				{#snippet icon()}<BrightnessIcon />{/snippet}
			</ControlSlider>
		</ActionCenterTile>
	</ActionCenterSurface>
	...
	>
		<ActionCenterTile grid={[1, 1]} role="region">
			<ControlSlider bind:value={display.volume} label="Sonido">
				{#snippet icon()}<VolumeIcon />{/snippet}
			</ControlSlider>
		</ActionCenterTile>
	</ActionCenterSurface>
</section>
