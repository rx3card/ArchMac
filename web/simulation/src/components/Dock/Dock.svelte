...
<script lang="ts">
	...
	import { apps } from '$lib/state/apps.svelte';
	import { system_needs_update } from '$lib/state/system.svelte';
	import { is_dock_hidden } from '$lib/state/dock.svelte';
...
>
	...
	>
		{#each Object.entries(apps_config).filter(([, c]) => !c.dock_hidden) as [appID, config]}
